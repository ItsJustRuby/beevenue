...
class _AnimatedThumbnailTemporaryDirectory(AbstractContextManager):
    ...
    def filename(self, local_path: str):
...
def _entire(in_path: str, medium_hash: str) -> int:
...
def generate_animated_task(in_path: str, medium_hash: str) -> None:
    ...
    if in_path.endswith(".gif"):
        return _entire(in_path, medium_hash)
    ...
    best_scenes = _pick_scenes(scene_list, length_in_ms, N)

    # pick K seconds starting at each pin
    for thumbnail_size, thumbnail_size_pixels in current_app.config[
        "BEEVENUE_THUMBNAIL_SIZES"
    ].items():
        with _AnimatedThumbnailTemporaryDirectory() as d:
            i = 0
            for scene in best_scenes:
                scene_length_in_milliseconds = (scene[1] - scene[0]).get_seconds() * 1000
                if scene_length_in_milliseconds < K * 1000:
                    trim_in_milliseconds = scene_length_in_milliseconds
                else:
                    trim_in_milliseconds = K * 1000
            ...
            playlist_filename = d.filename("temp.txt")
